...
ROART_Leaf::ROART_Leaf(uint8_t* key_, size_t key_len_, char* value_, size_t val_len_) : BaseNode(NTypes::ROART_Leaf) {
    key_len = key_len_;
    val_len = val_len_;
#ifdef KEY_INLINE
...
uint64_t N::getVersion() const {
    return type_version_lock_obsolete->load();
}
...
uintptr_t N::makeZentry(uint8_t key, N* node) {
    return (uintptr_t(key) << ZentryKeyShift) | reinterpret_cast<uintptr_t>(node);
}
    ...
    while (i < LeafArrayLength) {
        ...
        if (fingerprint_ptr != 0) {
            ...
            auto ptr = reinterpret_cast<ROART_Leaf*>(
              fingerprint_ptr ^ (static_cast<uintptr_t>(thisfp) << FingerPrintShift));
...
bool LeafArray::insert(ROART_Leaf* l, bool flush) {
    ...
    if (pos < LeafArrayLength) {
        ...
        bitmap.store(b);
        auto s =
          (static_cast<uintptr_t>(l->getFingerPrint()) << FingerPrintShift) | (reinterpret_cast<uintptr_t>(l));
#ifdef ROART_PROFILE_TIME
        gettimeofday(&start_time, NULL);
#endif
        leaf[pos].store(s);
...
uint32_t LeafArray::getCount() const {
    return bitmap.load().count();
}

bool LeafArray::isFull() const {
    return getCount() == LeafArrayLength;
}
