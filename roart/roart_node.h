...
protected:
    N(NTypes type, uint32_t level, const uint8_t* prefix, uint32_t prefixLength) : BaseNode(type), level(level) {
    ...
    N(NTypes type, uint32_t level, const Prefix& prefi) : BaseNode(type), prefix(prefi), level(level) {
        type_version_lock_obsolete = new std::atomic<uint64_t>;
        type_version_lock_obsolete->store(0b100);
        recovery_latch.store(0, std::memory_order_seq_cst);
        setType(type);
...
public:
    ...
    static inline bool isDirty(N* val) {
