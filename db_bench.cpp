...
class FastFairTester : public DataStructureTester {
private:
    fastfair* ff_dense_ = nullptr;
    fastfair* ff_sparse_ = nullptr;
    fastfair* ff_clustered_ = nullptr;
    fastfair* ff_variable_ = nullptr;
    fastfair* ff_ycsb_ = nullptr;

public:
    FastFairTester(LogManager& log) : DataStructureTester("FAST&FAIR", log) {}
...
protected:
    bool initialize() override {
        ...
        if (g_config.test_ycsb) {
            ff_ycsb_ = new_fastfair();
            if (ff_ycsb_ == nullptr) {
                log_.log_error("YCSB FAST&FAIR 实例创建失败");
                success = false;
            } else {
                log_.log_info("  |-> 创建 YCSB 数据集实例对象", "ff_ycsb_");
            }
        } else {
            if (g_config.test_dense) {
                ff_dense_ = new_fastfair();
                if (ff_dense_ == nullptr) {
                    log_.log_error("Dense FAST&FAIR 实例创建失败");
                    success = false;
                } else {
                    log_.log_info("  |-> 创建 Dense 数据集实例对象", "ff_dense_");
                }
            }

            if (g_config.test_sparse) {
                ff_sparse_ = new_fastfair();
                if (ff_sparse_ == nullptr) {
                    log_.log_error("Sparse FAST&FAIR 实例创建失败");
                    success = false;
                } else {
                    log_.log_info("  |-> 创建 Sparse 数据集实例对象", "ff_sparse_");
                }
            }

            if (g_config.test_clustered) {
                ff_clustered_ = new_fastfair();
                if (ff_clustered_ == nullptr) {
                    log_.log_error("Clustered FAST&FAIR 实例创建失败");
                    success = false;
                } else {
                    log_.log_info("  |-> 创建 Clustered 数据集实例对象", "ff_clustered_");
                }
            }
        }
    ...
    void test_insert(uint64_t* keys, int count, PerformanceMetrics::DatasetType type, double& time_us) override {
        fastfair* ff = static_cast<fastfair*>(get_instance(type));

        time_us = measure_time([&]() {
            for (int i = 0; i < count; i++) {
                ff->put(keys[i], (char*)&i);
                // show_progress(i + 1, count, "插入");
            }
        });
        // int i = 2, j = 4;
        // ff->put(keys[1], (char*)&i);
        // printf("ff->get(1) = %p, i = %p, *i = %d\n", ff->get(keys[1]), (char*)&i, *(char*)&i);
        // ff->put(keys[1], (char*)&j);
        // printf("ff->get(1) = %p, j = %p, *j = %d\n", ff->get(keys[1]), (char*)&j, *(char*)&j);

        // int num = count / 10;
        // for (int i = 0, j = num * i; i < 10; i++) {
        //     time_us = measure_time([&]() {
        //         for (int k = 0; k < num; k++, j++) {
        //             ff->put(keys[j], (char*)&j);
        //             // show_progress(j + 1, count, "插入");
        //         }
        //     });
        //     printf("  |-> FAST&FAIR %s insert 执行第 %d 个 %llu 次操作耗时: %.f ns, 吞吐量: %.6f Mops\n",
        //            dataset_display_name(type).c_str(), i + 1, num, time_us * 1000,
        //            num / (time_us / 1000000.0) / 1000000.0);
        // }

        // printf("  |-> FAST&FAIR %s insert %llu 操作总耗时: %.f ns\n",
        //        dataset_display_name(type).c_str(), count, time_us * 1000);

        // uint64_t total_update_latency = std::accumulate(ff_per_insert_node_update_latency_.begin(), ff_per_insert_node_update_latency_.end(), 0ULL);

        // uint64_t total_split_grow_latency = std::accumulate(ff_per_insert_node_split_latency_.begin(), ff_per_insert_node_split_latency_.end(), 0ULL);

        // uint64_t total_traversal_latency = time_us * 1000 - total_update_latency - total_split_grow_latency;

        // printf("  |-> FAST&FAIR %s insert 每次插入平均耗时: %.f ns, 树遍历延迟平均延迟: %llu ns, 节点分裂+节点扩容的平均延迟: %llu ns,  节点更新的平均延迟: %llu ns\n",
        //        dataset_display_name(type).c_str(),
        //        time_us * 1000 / count,
        //        total_traversal_latency / count,
        //        total_split_grow_latency / count,
    ...
    void test_search(uint64_t* keys, int count, PerformanceMetrics::DatasetType type, double& time_us) override {
        ...
        time_us = measure_time([&]() {
            for (int i = 0; i < count; i++) {
                ff->get(keys[i]);
                // unique_FASTFAIR_keys.insert(ff->get(keys[i]));
                // show_progress(i + 1, count, "查询");
            }
        });

        printf("  |-> FAST&FAIR %s search 操作返回的唯一值数量: %llu, 总耗时: %.f s\n",
               dataset_display_name(type).c_str(), unique_FASTFAIR_keys.size(), time_us * 1000);

        printf("  |-> FAST&FAIR %s search 操作访问节点 %llu, 访问每个节点的平均延迟: %llu ns, 平均访问节点的数量: %.3f\n",
               dataset_display_name(type).c_str(),
               ff_per_search_per_node_latency_.size(),
               std::accumulate(ff_per_search_per_node_latency_.begin(), ff_per_search_per_node_latency_.end(), 0ULL) / (ff_per_search_per_node_latency_.size() ? ff_per_search_per_node_latency_.size() : 1),
               static_cast<double>(ff_per_search_per_node_latency_.size()) / count);
    }
    ...
    void test_range(uint64_t start, uint64_t end, PerformanceMetrics::DatasetType type, double& time_us, uint64_t& result_count) override {
        ...
        time_us = measure_time([&]() {
            result_count = ff->scan(start, end).size();
        });

        printf("  |-> FAST&FAIR %s 查询范围 [%llu, %llu] 共 %llu 个键\n", dataset_display_name(type).c_str(), start, end, result_count);
    }
    ...
    void test_ycsb_operations(YCSBLoader* loader, int count, YCSBPhase phase, PerformanceMetrics::DatasetType ycsb_type, double& time_us) override {
        fastfair* ff = static_cast<fastfair*>(get_instance(ycsb_type));
        auto& operation_records = loader->get_operation_records(phase);

        time_us = measure_time([&]() {
            for (int i = 0; i < count; i++) {
                if (operation_records[i].op_char == 'r') {
                    ff->get(operation_records[i].key);
                } else if (operation_records[i].op_char == 'u' || operation_records[i].op_char == 'i') {
                    ff->put(operation_records[i].key, (char*)&i);
                }
    ...
    uint64_t get_memory_usage(PerformanceMetrics::DatasetType type) override {
        ...
        return ff->memory_profile(nullptr);
    }
...
// class WOPETester : public DataStructureTester {
// private:
//     wope* wope_dense_ = nullptr;
//     wope* wope_sparse_ = nullptr;
//     wope* wope_clustered_ = nullptr;
//     wope* wope_ycsb_ = nullptr;
// public:
//     WOPETester(LogManager& log) : DataStructureTester("WOPE", log) {}
//     ~WOPETester() {
//         // 建议添加析构逻辑以释放内存，如果 wope 类有对应的销毁函数
//         // delete wope_dense_; ...
//     }
// protected:
//     bool initialize() override {
...
//         if (g_config.test_ycsb) {
//             wope_ycsb_ = new wope();
//             if (wope_ycsb_ == nullptr) {
//                 log_.log_error("YCSB WOPE 实例创建失败");
//                 success = false;
//             } else {
//                 log_.log_info("  |-> 创建 YCSB 数据集实例对象", "wope_ycsb_");
//             }
//         } else {
//             if (g_config.test_dense) {
//                 wope_dense_ = new wope();
//                 if (wope_dense_ == nullptr) {
//                     log_.log_error("Dense WOPE 实例创建失败");
//                     success = false;
//                 } else {
//                     log_.log_info("  |-> 创建 Dense 数据集实例对象", "wope_dense_");
//                 }
//             }
//             if (g_config.test_sparse) {
//                 wope_sparse_ = new wope(); // 修正：new wope()
//                 if (wope_sparse_ == nullptr) {
//                     log_.log_error("Sparse WOPE 实例创建失败");
//                     success = false;
//                 } else {
//                     log_.log_info("  |-> 创建 Sparse 数据集实例对象", "wope_sparse_");
//                 }
//             }
//             if (g_config.test_clustered) {
//                 wope_clustered_ = new wope(); // 修正：new wope()
//                 if (wope_clustered_ == nullptr) {
//                     log_.log_error("Clustered WOPE 实例创建失败");
//                     success = false;
//                 } else {
//                     log_.log_info("  |-> 创建 Clustered 数据集实例对象", "wope_clustered_");
//                 }
//             }
//         }
//         return success;
...
//     void test_insert(uint64_t* keys, int count, PerformanceMetrics::DatasetType type, double& time_us) override {
//         // 修正类型：wope*
//         wope* wope_obj = static_cast<wope*>(get_instance(type));
//         time_us = measure_time([&]() {
//             for (int i = 0; i < count; i++) {
//                 wope_obj->insert(static_cast<entry_key_t>(keys[i]), (void*)(uintptr_t)i);
//             }
//         });
//     }
//     void test_search(uint64_t* keys, int count, PerformanceMetrics::DatasetType type, double& time_us) override {
...
//         time_us = measure_time([&]() {
//             for (int i = 0; i < count; i++) {
//                 // 修正接口：直接调用 wope_obj->search
//                 // 返回值是 entry_key_t，这里强制转为 uint64_t 防止编译警告
//                 volatile uint64_t result = static_cast<uint64_t>(wope_obj->search(static_cast<entry_key_t>(keys[i])));
//                 // 防止编译器优化掉 search 调用
//                 if (result == 0 && false)
//                     asm volatile("");
//             }
...
//         wope* wope_obj = static_cast<wope*>(get_instance(type));
//         time_us = measure_time([&]() {
//             // 修正接口：wope->scan 定义为 void scan(entry_key_t, entry_key_t, long)
//             // 第三个参数是 len，这里假设传入 8 或者根据需求调整
//             wope_obj->scan(static_cast<entry_key_t>(start), static_cast<entry_key_t>(end), 8);
//             result_count = 1; // 简单处理
//         });
//     }
//     void test_ycsb_operations(YCSBLoader* loader, int count, YCSBPhase phase, PerformanceMetrics::DatasetType ycsb_type, double& time_us) override {
//         wope* wope_obj = static_cast<wope*>(get_instance(ycsb_type));
//         auto& operation_records = loader->get_operation_records(phase);
//         time_us = measure_time([&]() {
//             for (int i = 0; i < count; i++) {
//                 uint64_t key = operation_records[i].key;
//                 if (operation_records[i].op_char == 'r') {
//                     // 读操作
//                     wope_obj->search(static_cast<entry_key_t>(key));
//                 } else if (operation_records[i].op_char == 'u' || operation_records[i].op_char == 'i') {
//                     // 写/更新操作
//                     // 注意：wope::insert 第二个参数是 void*，这里强转 value
//                     wope_obj->insert(static_cast<entry_key_t>(key), (void*)(uintptr_t)operation_records[i].value);
//                 }
//             }
//         });
//     }
//     uint64_t get_memory_usage(PerformanceMetrics::DatasetType type) override {
//         // 注意：Cre.txt 中没有提供直接的内存计算接口
//         // 这里返回 0 或者根据全局变量（如 alloer）估算
//         // 如果 wope 类没有暴露内存统计方法，只能返回占位符
//         return 0;
//     }
//     void* get_instance(PerformanceMetrics::DatasetType type) override {
//         switch (type) {
...
class LBTreeTester : public DataStructureTester {
private:
    lbtree* lb_dense_ = nullptr;
    lbtree* lb_sparse_ = nullptr;
    lbtree* lb_clustered_ = nullptr;
    lbtree* lb_ycsb_ = nullptr;

public:
    LBTreeTester(LogManager& log) : DataStructureTester("LB+Tree", log) {}
...
protected:
    bool initialize() override {
        ...
        if (g_config.test_ycsb) {
            lb_ycsb_ = new_lbtree();
            if (lb_ycsb_ == nullptr) {
                log_.log_error("YCSB LB+Tree 实例创建失败");
                success = false;
            } else {
                log_.log_info("  |-> 创建 YCSB 数据集实例对象", "lb_ycsb_");
            }
        } else {
            if (g_config.test_dense) {
                lb_dense_ = new_lbtree();
                if (lb_dense_ == nullptr) {
                    log_.log_error("Dense LB+Tree 实例创建失败");
                    success = false;
                } else {
                    log_.log_info("  |-> 创建 Dense 数据集实例对象", "lb_dense_");
                }
            }

            if (g_config.test_sparse) {
                lb_sparse_ = new_lbtree();
                if (lb_sparse_ == nullptr) {
                    log_.log_error("Sparse LB+Tree 实例创建失败");
                    success = false;
                } else {
                    log_.log_info("  |-> 创建 Sparse 数据集实例对象", "lb_sparse_");
                }
            }

            if (g_config.test_clustered) {
                lb_clustered_ = new_lbtree();
                if (lb_clustered_ == nullptr) {
                    log_.log_error("Clustered LB+Tree 实例创建失败");
                    success = false;
                } else {
                    log_.log_info("  |-> 创建 Clustered 数据集实例对象", "lb_clustered_");
                }
            }
        }
    ...
    void test_insert(uint64_t* keys, int count, PerformanceMetrics::DatasetType type, double& time_us) override {
        lbtree* lb = static_cast<lbtree*>(get_instance(type));

        time_us = measure_time([&]() {
            for (int i = 0; i < count; i++) {
                lb->insert(keys[i], (char*)&i);
                // show_progress(i + 1, count, "插入");
            }
        });
    }
    ...
    void test_search(uint64_t* keys, int count, PerformanceMetrics::DatasetType type, double& time_us) override {
        ...
        time_us = measure_time([&]() {
            for (int i = 0; i < count; i++) {
                lb->lookup(keys[i]);
                // unique_LBTree_keys.insert(lb->lookup(keys[i]));
                // show_progress(i + 1, count, "查询");
            }
        });

        printf("  |-> LB+Tree %s search 操作返回的唯一值数量: %llu, 耗时: %f us\n", dataset_display_name(type).c_str(), unique_LBTree_keys.size(), time_us);
    }
    ...
    void test_range(uint64_t start, uint64_t end, PerformanceMetrics::DatasetType type, double& time_us, uint64_t& result_count) override {
        ...
        time_us = measure_time([&]() {
            result_count = lb->rangeQuery(start, end).size();
        });

        printf("  |-> LB+Tree %s 查询范围 [%llu, %llu] 共 %llu 个键, 耗时: %f us\n", dataset_display_name(type).c_str(), start, end, result_count, time_us);
    }

    void test_ycsb_operations(YCSBLoader* loader, int count, YCSBPhase phase, PerformanceMetrics::DatasetType ycsb_type, double& time_us) override {
        lbtree* lb = static_cast<lbtree*>(get_instance(ycsb_type));
        auto& operation_records = loader->get_operation_records(phase);

        time_us = measure_time([&]() {
            for (int i = 0; i < count; i++) {
                if (operation_records[i].op_char == 'r') {
                    lb->lookup(operation_records[i].key);
                } else if (operation_records[i].op_char == 'u' || operation_records[i].op_char == 'i') {
                    lb->insert(operation_records[i].key, (char*)&operation_records[i].value);
                }
